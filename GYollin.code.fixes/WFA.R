...
## ----TESTOPT-------------------------------------------------------------
chart.forward("wfa.results.RData", portfolio.name = "opt")
# prints
# "Error in chart.forward("wfa.results.RData") : object '.audit' not found"

# if one loads this env file first
load("wfa.results.RData")
# this creates the audit env -- what's the point of reporting an error
# if the .audit exists in the file being loaded ?

# the following should produce the chart
chart.forward("wfa.results.RData", portfolio.name = "opt")

# final note: judging by the help documentation,
# there is no difference b/n chart.forward and chart.forward.training
# 
...
if(0){
    e1 <- new.env()
    load("wfa.results.RData", envir = e1)
    load("wfa.results.RData")
}
